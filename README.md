...
> DeepSeek Harness 移植版 [pi-tokensaver](https://github.com/xilnick/pi-tokensaver)（[Pi coding agent](https://github.com/mariozechner/pi-coding-agent) 扩展）——
> 把 [tokensave](https://github.com/nicr9/tokensave) 的本地语义图引擎接进 DSH，让 agent 用语义查询代替暴力 grep/读文件，大幅省 token。
...
1. [tokensave](https://github.com/nicr9/tokensave) 已安装（`cargo install tokensave`，或从 GitHub Releases 下载）。
...
# 2. 在 $DSH_HOME/profiles/web/cordis.patch.yml 挂载（cli 模式只需一行）：
```
...
> mcp 模式需要两行（顺序不能反）：先本插件（`bridge: mcp` 或省略），
> 再一行 `@deepseek-ai/dsh-mcp-client` 指向 `tokensave serve`，工具名带 `mcp__` 前缀。

重启（或热重载）`dsh web` 后，模型即可调用 `tokensave_search`、`tokensave_callers` 等工具。
...
```bash
# MCP 冒烟测试（验证 tokensave serve 的 MCP 协议与工具可用性）
...
- [tokensave](https://github.com/nicr9/tokensave) — Rust 语义图引擎
